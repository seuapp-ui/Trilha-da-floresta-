...
export class HUD {
  ...
  _buildHearts(count) {
    for (let i = 0; i < count; i++) {
      const heart = this.scene.add.image(24 + i * 26, 24, 'heart').setScrollFactor(0).setDepth(GameConfig.DEPTH.HUD).setScale(1.3);
      this.hearts.push(heart);
    }
  }

  _buildCounters(coins, crystals) {
    const style = { fontFamily: 'Arial, sans-serif', fontSize: '20px', color: '#ffffff', stroke: '#00000080', strokeThickness: 4 };

    this.coinIcon = this.scene.add.image(24, 58, 'icon_coin').setScrollFactor(0).setDepth(GameConfig.DEPTH.HUD);
    this.coinText = this.scene.add.text(40, 48, `${coins}`, style).setScrollFactor(0).setDepth(GameConfig.DEPTH.HUD);

    this.crystalIcon = this.scene.add.image(24, 90, 'icon_crystal').setScrollFactor(0).setDepth(GameConfig.DEPTH.HUD);
    this.crystalText = this.scene.add.text(40, 80, `${crystals}`, style).setScrollFactor(0).setDepth(GameConfig.DEPTH.HUD);
  }
